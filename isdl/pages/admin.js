...
const admin = () => {
  ...
  async function bookHall({ pending, ac }) {
    const response = await fetch("https://isdl-backend2.onrender.com/api/acceptRequest?", {
      method: "POST",
      headers: {
  ...
  async function getPendings() {
    const response = await fetch("https://isdl-backend2.onrender.com/api/getAllPending", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
